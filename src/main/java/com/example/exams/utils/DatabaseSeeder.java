...
public class DatabaseSeeder implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        // ADMINISTRATORZY – id jako String
        ...
        // Dodatkowi egzaminatorzy – id jako String
        ...
        // EGZAMINY – id jako String, subject findById z String
        examRepository.save(new Exam(
                "1",
                6,
                false,
                "Projektowanie części w SOLIDWORKS",
                LocalDate.now(),
                LocalTime.now().withNano(0),
                LocalDate.now(),
                LocalTime.now().plusHours(2).withNano(0),
                subjectRepository.findById("1").get(),
                students1
        ));
        ...
        examRepository.save(new Exam(
                ...
                LocalTime.now().minusHours(4).withNano(0),
                LocalDate.now(),
                LocalTime.now().plusHours(1).withNano(0),
                subjectRepository.findById("1").get(),
                students1
        ));
        ...
        examRepository.save(new Exam(
                ...
                0,
                false,
                "Jak zrobić sprawozdanie w MS Paint",
                LocalDate.now().minusDays(2),
                LocalTime.now().plusHours(2).withNano(0),
                LocalDate.now(),
                LocalTime.now().plusHours(5).withNano(0),
                subjectRepository.findById("3").get(),
                students2
        ));
        ...
        examRepository.save(new Exam(
                ...
                "Uzywanie Autocada",
                LocalDate.now(),
                LocalTime.now().minusHours(1).withNano(0),
                LocalDate.now(),
        ...
        examRepository.save(new Exam(
                ...
                0,
                false,
                "Jak zrobić sprawozdanie w notatniku",
                LocalDate.now().minusDays(2),
                LocalTime.now().withSecond(0).withNano(0),
                LocalDate.now(),
                LocalTime.now().plusHours(1).withSecond(0).withNano(0),
        ...
        // PYTANIA OTWARTE – id jako String, exam przez findById z String
        openQuestionRepository.save(new OpenQuestion("1", "Ile to 5+5?", 10, examRepository.findById("1").get()));
        openQuestionRepository.save(new OpenQuestion("2", "Ile to 30*3", 10, examRepository.findById("1").get()));
        openQuestionRepository.save(new OpenQuestion("3", "Ile to 19+3?", 10, examRepository.findById("1").get()));

        openQuestionRepository.save(new OpenQuestion("4", "Ile to 1+1?", 5, examRepository.findById("2").get()));
        openQuestionRepository.save(new OpenQuestion("5", "Ile to 2*2", 5, examRepository.findById("2").get()));
        openQuestionRepository.save(new OpenQuestion("6", "Ile to 3+3?", 5, examRepository.findById("2").get()));

        // PYTANIA ZAMKNIĘTE
        closedQuestionRepository.save(new Closedquestion("1", "ile to jest 10*10", 1, examRepository.findById("1").get()));
        closedQuestionRepository.save(new Closedquestion("2", "ile to jest 11*11", 1, examRepository.findById("1").get()));
        closedQuestionRepository.save(new Closedquestion("3", "ile to jest 12*12", 1, examRepository.findById("1").get()));

        closedQuestionRepository.save(new Closedquestion("4", "ile to 4*4", 2, examRepository.findById("2").get()));
        closedQuestionRepository.save(new Closedquestion("5", "ile to 5*5", 2, examRepository.findById("2").get()));
        closedQuestionRepository.save(new Closedquestion("6", "ile to 6*6", 2, examRepository.findById("2").get()));

        // ODPOWIEDZI ZAMKNIĘTE – id jako String
        ...
        // EXAMY DO LOGÓW – findById z String
        Exam exam1Entity = examRepository.findById("1").orElse(null);
        Exam exam2Entity = examRepository.findById("2").orElse(null);

        if (exam1Entity != null && exam2Entity != null) {
            ...
            // LOGI STUDENT-EXAM – id jako String
            logstudentexamRepository.save(new Logstudentexam("1", "Description 1", LocalDate.now(), LocalTime.now(), scoreResult1, exam1Entity, student1));
            logstudentexamRepository.save(new Logstudentexam("2", "Description 2", LocalDate.now(), LocalTime.now(), scoreResult2, exam1Entity, student2));
            logstudentexamRepository.save(new Logstudentexam("3", "Description 3", LocalDate.now(), LocalTime.now(), scoreResult3, exam2Entity, student3));
            logstudentexamRepository.save(new Logstudentexam("4", "Description 4", LocalDate.now(), LocalTime.now(), scoreResult4, exam2Entity, student4));
        }
    }
